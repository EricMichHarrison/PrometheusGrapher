    ...
    while (true) {
        ...
        if (screenUpdate) {
            ...
            for (int i = 1; i < lines; i++) {
                M5.Display.drawLine(0, lineStep * i, maxx, lineStep * i,
                                    SystemColour);
            }
            screenUpdate = false;
        }
        if (M5Cardputer.Keyboard.isChange()) {
            ...
            if (currentMillis - lastDebounceTime >=
                debounceDelay) { // prevent acidental double click
...
void DrawGraphPage() {
    ...
    while (true) {
        ...
        if (oldPosX != posx || oldPosY != posy) {
            M5.Display.clear();
            M5Cardputer.Display.drawLine(cx - posx, 0, cx - posx, maxy,
                                         SystemColour);
            M5Cardputer.Display.drawLine(0, cy + posy, maxx, cy + posy,
                                         SystemColour);
            oldPosX = posx;
            oldPosY = posy;
            ...
            for (int i = lineStepCount; i < maxx - posx;
                ...
                M5Cardputer.Display.drawLine(cx - posx - i, 0, cx - posx - i,
                                             maxy, StepColour);
            }
            for (int i = lineStepCount; i < maxx + posx;
            ...
            for (int i = lineStepCount; i < maxy + posy;
                 i += lineStepCount) { // top
                M5Cardputer.Display.drawLine(0, cy + posy - i, maxx,
                                             cy + posy - i, StepColour);
            }
            for (int i = lineStepCount; i < maxy - posy;
